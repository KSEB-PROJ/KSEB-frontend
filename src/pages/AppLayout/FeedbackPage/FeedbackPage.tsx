...
const FeedbackPage: React.FC = () => {
    ...
    const feedbackText = "이번 발표에서 여러 강점들이 돋보였습니다.\n 먼저, 발표 내용을 체계적으로 잘 구성하여 청중이 내용을 따라가기 쉬웠으며, 각 주요 포인트가 명확하게 전달되어 이해도를 높였습니다. 특히 서론에서 주제를 분명히 제시하고, 중간에 관련 사례와 데이터를 적절히 활용해 신뢰성을 더한 점이 좋았습니다.\n 발표자의 표정은 대체로 자연스러웠으나, 중간중간 긴장한 듯한 모습이 보였습니다. 앞으로는 더 자신감을 표현할 수 있도록 미소를 자주 짓고, 시선을 청중과 자주 맞추는 연습이 필요하겠습니다.\n 또한 동작 면에서는 손짓이 다소 제한적이었는데, 중요한 내용을 강조할 때 자연스러운 제스처를 추가하면 메시지가 더욱 효과적으로 전달될 것입니다.\n 말투는 대체로 명확하고 또박또박했지만, 몇몇 부분에서 말하는 속도가 빨라져 청중이 따라가기 어려웠던 점이 있었습니다. 적절한 속도 조절과 중간중간 пауз(멈춤)를 넣어 청중이 내용을 소화할 시간을 줄 필요가 있습니다. 또한 단조로움을 줄이기 위해 높낮이 변화와 감정 전달에도 신경 써보면 좋겠습니다.\n 마지막으로, 기술적인 면에서는 발표 자료의 시각 요소(슬라이드 디자인, 글자 크기, 색상 대비 등)가 전체적으로 깔끔했으나, 일부 그래프와 텍스트가 조밀하게 배치되어 가독성이 떨어진 부분이 있었습니다. 쉬운 이해를 돕기 위해 핵심 내용 중심으로 간결하게 수정하면 발표 효과가 더욱 증가할 것입니다.\n 종합하면, 발표자의 노력과 전문성이 돋보이는 발표였으며, 비언어적 표현과 속도 조절에 조금 더 신경 쓴다면 훨씬 자연스럽고 설득력 있는 발표가 될 것으로 기대됩니다. 계속해서 연습과 피드백을 통해 개선해나가시길 바랍니다.";
    const[lastfeedbacks, setLastFeedbacks] = useState<{ id: number; timestamp:string} []>([]);
    const navigate = useNavigate();
    const handleAddFeedback = () => {
        if (lastfeedbacks.length >= 10)
            return;
        const now = new Date();
        setLastFeedbacks([
            ...lastfeedbacks,{
                id:lastfeedbacks.length + 1,
            ...
            <div className={styles.bottom}>
                <div className={styles.line}>
                        ...
                        >
                            ...
                            <div className={styles.tooltip}>{f.timestamp}</div>
                        </div>
                        ))}
